  ...
  <div class="container">
    ...
    <div class="table">
      ...
      >
        ...
        >
        <el-table-column
          prop="businessExecutionPosition"
          label="岗位名称"
          width="140"
        ></el-table-column>
        <!-- <el-table-column
        ...
        <el-table-column label="关联问题" width="120">
          ...
          >
            <span v-if="scope.row.problemBase">
              {{ scope.row.problemBase.detailedDescription }}</span
            >
            <span v-else> - </span>
          </div>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    downloadmodel() {
      getModle(
        "http://10.84.1.223:9000/gy-file/2023/10/16/非执法领域内控库导入模板/2104/非执法领域内控库导入模板.xlsx"
      );
    },
    disable(data) {
    ...
    muntiedetile() {
      this.$confirm("是否批量删除已选数据", {
        ...
        delateinarea(this.deletelist).then((res) => {
          const message = res.msg;
          this.$notify({
            title: "成功",
            message: message,
